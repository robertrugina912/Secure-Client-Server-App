...
void trim_str(char *str) {
  ...
  for(i = 0; i < leading_spaces; i++) {
    ...
    printf("SHIFTING loop 2\n");
  }
  if(trailing_spaces > 0) str[len-trailing_spaces+1] = '\0';
}
...
assert(state);
  ...
  if(strcmp(buf,"/help") == 0) {
    ...
    if(strcmp(arg[1],"") == 0 || strcmp(arg[2],"") == 0 || space_count != 2) {
      printf("error: invalid command format\n");
      state->command = CMD_DO_NOTHING;
    } else {
    ...
    if(
      strcmp(arg[1],"") == 0 || strcmp(arg[2],"") == 0 || space_count != 2
    ) {
      printf("error: invalid command format\n");
      state->command = CMD_DO_NOTHING;
    } else {
